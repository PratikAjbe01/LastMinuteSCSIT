...
import { User, Mail, FileText, Calendar, Book, Tag, Edit, Trash2, X, Loader, AlertCircle, ShieldCheck, GraduationCap, FileX, Upload, View } from "lucide-react"
...
const MyFilesPage = () => {
    ...
    useEffect(() => {
        ...
        const fetchFiles = async () => {
            ...
            setError(null);
            try {
                const response = await fetch('http://localhost:5000/api/files/adminfiles', {
                    method: 'POST',
                    headers: {
    ...
    const handleUpdateFile = async (e) => {
        ...
        setModalError('');
        try {
            const response = await fetch('http://localhost:5000/api/files/update', {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${user.token}`,
