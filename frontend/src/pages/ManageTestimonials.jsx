      ...
      const response = await fetch(
        `${API_URL}/api/testimonials/uploadtestimonial`,
        {
          ...
          body: JSON.stringify({
            ...
            userProfile: profileInputType === "link" ? profileLink : profileUploadUrl || userProfileUrl,
          }),
        },
      );
      ...
      <AnimatePresence>
            ...
            >
              <form onSubmit={handleReviewSubmit} className="!z-50 relative">
                ...
                <div className="p-8 space-y-6 h-[calc(100vh-240px)] overflow-y-scroll">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="relative">
                      ...
                      <input type="text" value={authorName} onChange={(e) => setAuthorName(e.target.value)} placeholder="Author's Name" required className="w-full rounded-lg border border-gray-600 bg-gray-800 py-2.5 pl-10 pr-4 text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-green-500" />
                    </div>
                    <div className="relative">
                      <Mail className="absolute left-3 top-1/2 -translate-y-1/2 h-5 w-5 text-gray-500" />
                      <input type="email" value={authorEmail} onChange={(e) => setAuthorEmail(e.target.value)} placeholder="Author's Email" required className="w-full rounded-lg border border-gray-600 bg-gray-800 py-2.5 pl-10 pr-4 text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-green-500" />
                    </div>
                  </div>

                  <div className="relative">
                    <Briefcase className="absolute left-3 top-1/2 -translate-y-1/2 h-5 w-5 text-gray-500" />
                    <input type="text" value={authorProfession} onChange={(e) => setAuthorProfession(e.target.value)} placeholder="Profession (e.g., Software Engineer)" className="w-full rounded-lg border border-gray-600 bg-gray-800 py-2.5 pl-10 pr-4 text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-green-500" />
                  </div>

                  <div>
                    <textarea value={feedback} onChange={(e) => setFeedback(e.target.value)} rows={5} placeholder="Share your experience in detail..." required className="w-full rounded-lg border border-gray-600 bg-gray-800 p-3 text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-green-500" />
                  </div>
                    ...
                    <div>
                      ...
                      <div className="flex gap-2">
                        {["Good", "Outstanding"].map((r) => (
                          <label key={r} className={`flex-1 cursor-pointer rounded-lg p-2 text-center text-sm font-semibold transition-all ${rating === r ? 'bg-green-600 text-white' : 'bg-gray-700/50 hover:bg-gray-700'}`}>
                            <input type="radio" name="rating" value={r} checked={rating === r} onChange={(e) => setRating(e.target.value)} className="sr-only" />{r}
                          </label>))}
                      </div>
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-gray-300 mb-3">Important Label</label>
                      <div className="flex gap-2">
                        <label className={`flex-1 cursor-pointer rounded-lg p-2 text-center text-sm font-semibold transition-all ${isImportant ? 'bg-green-600 text-white' : 'bg-gray-700/50 hover:bg-gray-700'}`}>
                          <input type="radio" name="isImportant" checked={isImportant} onChange={() => setIsImportant(true)} className="sr-only" />Yes
                        </label>
                        <label className={`flex-1 cursor-pointer rounded-lg p-2 text-center text-sm font-semibold transition-all ${!isImportant ? 'bg-red-600 text-white' : 'bg-gray-700/50 hover:bg-gray-700'}`}>
                          <input type="radio" name="isImportant" checked={!isImportant} onChange={() => setIsImportant(false)} className="sr-only" />No
                        </label>
                      </div>
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="flex rounded-lg bg-gray-800 p-1">
                      <button type="button" onClick={() => setProfileInputType("link")} className={`w-1/2 rounded-md py-2 text-sm font-semibold flex items-center justify-center gap-2 transition-colors ${profileInputType === "link" ? "bg-green-600 text-white" : "text-gray-300 hover:bg-gray-700"}`}>
                        <Link size={16} /> Link
                      </button>
                      <button type="button" onClick={() => setProfileInputType("upload")} className={`w-1/2 rounded-md py-2 text-sm font-semibold flex items-center justify-center gap-2 transition-colors ${profileInputType === "upload" ? "bg-green-600 text-white" : "text-gray-300 hover:bg-gray-700"}`}>
                    ...
                    <div className="mt-3">
                      {profileInputType === "link" ? (
                        ...
                        <label className="w-full flex items-center justify-center gap-2 cursor-pointer rounded-lg border border-gray-600 bg-gray-800 p-2.5 text-gray-400 hover:bg-gray-700">
                          <FileText size={16} />
                          <span className="truncate max-w-xs">{profileFile ? profileFile.name : "Choose an image"}</span>
                          <input type="file" onChange={(e) => setProfileFile(e.target.files[0])} accept="image/*" className="sr-only" />
                        </label>
                      )}
                    </div>
                  </div>
                ...
                <div className="flex justify-end gap-4 bg-white/[.03] p-4">
                  <motion.button type="button" onClick={closeUploadModal} whileHover={{ scale: 1.03 }} disabled={isSubmittingReview} className="rounded-lg bg-gray-600 px-6 py-2 font-semibold text-white hover:bg-gray-700 disabled:opacity-50">Cancel</motion.button>
                  <motion.button type="submit" whileHover={{ scale: 1.03 }} disabled={isSubmittingReview} onClick={()=>handleReviewSubmit()} className="flex min-w-[100px] items-center justify-center rounded-lg bg-green-600 px-6 py-2 font-semibold text-white hover:bg-green-700 disabled:opacity-50">
                    {isSubmittingReview ? <Loader className="h-5 w-5 animate-spin" /> : "Submit"}
                  </motion.button>
                </div>
              </form>
            </motion.div>
          ...
          >
            ...
            >
              <div className="flex items-center justify-between border-b border-gray-700/50 bg-white/[.03] p-5">
                ...
                <button onClick={closeModal} className="text-gray-400 transition-colors hover:text-white"><X /></button>
              </div>
              <div className="grid gap-x-8 gap-y-6 p-8 md:grid-cols-2">
                <div className="space-y-5">
                  <div className="flex items-start gap-4"><div className="mt-1 flex-shrink-0 text-cyan-400"><User /></div><div><p className="text-sm text-gray-400">Author</p><div className="flex items-center gap-2"><p className="font-semibold text-white">{selectedTestimonial.user?.name || "Anonymous"}</p>{selectedTestimonial.isUserAdmin && (<span className="flex items-center gap-1 rounded-full bg-indigo-500/20 px-2 py-0.5 text-xs font-medium text-indigo-300"><Shield size={12} /> Admin</span>)}</div></div></div>
                  <div className="flex items-start gap-4"><div className="mt-1 flex-shrink-0 text-cyan-400"><Mail /></div><div><p className="text-sm text-gray-400">Email</p><p className="font-semibold text-white">{selectedTestimonial.user?.email || "N/A"}</p></div></div>
                  <div className="flex items-start gap-4"><div className="mt-1 flex-shrink-0 text-cyan-400"><BookOpen /></div><div><p className="text-sm text-gray-400">Academic Info</p><p className="font-semibold text-white">{selectedTestimonial.course || "N/A"} - Semester {selectedTestimonial.semester || "N/A"}</p></div></div>
                </div>
                <div className="space-y-5">
                  <div className="flex items-start gap-4"><div className="mt-1 flex-shrink-0">{selectedTestimonial.show === "yes" ? <CheckCircle className="text-green-400" /> : <Clock className="text-orange-400" />}</div><div><p className="text-sm text-gray-400">Status</p><p className={`font-semibold ${selectedTestimonial.show === "yes" ? "text-green-300" : "text-orange-300"}`}>{selectedTestimonial.show === "yes" ? "Approved" : "Pending"}</p></div></div>
                  <div className="flex items-start gap-4"><div className="mt-1 flex-shrink-0"><Star className={selectedTestimonial.rating === "Outstanding" ? "text-amber-400" : selectedTestimonial.rating === "Good" ? "text-sky-400" : "text-gray-500"} /></div><div><p className="text-sm text-gray-400">Rating</p><p className={`font-semibold ${selectedTestimonial.rating === "Outstanding" ? "text-amber-300" : selectedTestimonial.rating === "Good" ? "text-sky-300" : "text-gray-500"}`}>{selectedTestimonial.rating || "Not Rated"}</p></div></div>
                  <div className="flex items-start gap-4"><div className="mt-1 flex-shrink-0 text-cyan-400"><Calendar /></div><div><p className="text-sm text-gray-400">Timestamps</p><p className="font-semibold text-white">Submitted: {new Date(selectedTestimonial.createdAt).toLocaleString()}</p><p className="text-xs text-gray-500">Updated: {new Date(selectedTestimonial.updatedAt).toLocaleString()}</p></div></div>
                </div>
              </div>
              <div className="p-8 pt-0">
                <div className="space-y-2 rounded-xl border border-gray-700 bg-black/20 p-4">
                  <h4 className="text-sm font-semibold text-gray-300">Testimonial Message</h4>
                  <div className="max-h-32 overflow-y-auto pr-2">
                    <p className="italic text-gray-200">"{selectedTestimonial.message}"</p>
                  </div>
                </div>
              </div>
            </motion.div>
        ...
        {isConfirmModalOpen && selectedTestimonial && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 p-4 backdrop-blur-sm">
            <motion.div initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.9, y: 20 }} className="w-full max-w-md rounded-2xl border border-slate-700 bg-slate-800 p-8 text-center shadow-2xl">
              <div className={`mx-auto flex h-16 w-16 items-center justify-center rounded-full ${selectedTestimonial.show === "yes" ? "bg-orange-500/10" : "bg-green-500/10"}`}>
                {selectedTestimonial.show === "yes" ? <ThumbsDown className="h-8 w-8 text-orange-500" /> : <ThumbsUp className="h-8 w-8 text-green-500" />}
              </div>
              <h3 className="mt-4 text-2xl font-bold text-white">{selectedTestimonial.show === "yes" ? "Disapprove" : "Approve"} Testimonial?</h3>
              <p className="mt-2 text-gray-300">Are you sure you want to <strong className="text-white">{selectedTestimonial.show === "yes" ? "disapprove" : "approve"}</strong> this testimonial?</p>
              <div className="mt-6 flex flex-col justify-center gap-4 sm:flex-row">
                <motion.button type="button" onClick={closeModal} whileHover={{ scale: 1.03 }} disabled={isSubmitting} className="rounded-lg bg-gray-600 px-6 py-2 font-semibold text-white hover:bg-gray-700 disabled:opacity-50">Cancel</motion.button>
                <motion.button type="button" onClick={performShowUpdate} whileHover={{ scale: 1.03 }} disabled={isSubmitting} className={`flex items-center justify-center rounded-lg px-6 py-2 font-semibold disabled:opacity-50 ${selectedTestimonial.show === "yes" ? "bg-orange-600 text-white hover:bg-orange-700" : "bg-green-600 text-white hover:bg-green-700"}`}>
                  {isSubmitting ? <Loader className="h-5 w-5 animate-spin" /> : "Confirm"}
                </motion.button>
              </div>
            </motion.div>
          </motion.div>
        ...
        {isDeleteModalOpen && selectedTestimonial && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 p-4 backdrop-blur-sm">
            <motion.div initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.9, y: 20 }} className="w-full max-w-md rounded-2xl border border-slate-700 bg-slate-800 p-8 text-center shadow-2xl">
              ...
              <h3 className="mt-4 text-2xl font-bold text-white">Delete Testimonial?</h3>
              <p className="mt-2 text-gray-300">Are you sure you want to delete this testimonial? This action cannot be undone.</p>
              <div className="mt-6 flex flex-col justify-center gap-4 sm:flex-row">
                <motion.button type="button" onClick={closeModal} whileHover={{ scale: 1.03 }} disabled={isSubmitting} className="rounded-lg bg-gray-600 px-6 py-2 font-semibold text-white hover:bg-gray-700 disabled:opacity-50">Cancel</motion.button>
                <motion.button type="button" onClick={performDelete} whileHover={{ scale: 1.03 }} disabled={isSubmitting} className="flex items-center justify-center rounded-lg bg-red-600 px-6 py-2 font-semibold text-white hover:bg-red-700 disabled:opacity-50">
                  {isSubmitting ? <Loader className="h-5 w-5 animate-spin" /> : "Delete"}
                </motion.button>
              </div>
            </motion.div>
          </motion.div>
